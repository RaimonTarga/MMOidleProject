...
const out=resolve(here,'bot-t1-final');mkdirSync(resolve(out,'jobs'),{recursive:true});
const inventory=JSON.parse(readFileSync(resolve(here,'economy-inventory.json'),'utf8'));
const cases=[[1,'plains'],[1,'cave']];
copyFileSync(resolve(here,'run-bot-final.ts'),resolve(candidate,'reports/reward-mastery-study-2026-09-25/run-bot-final.ts'));
const jobs=[];
...
async function worker() {
  while(next<jobs.length) {
    ...
    const status=await new Promise(resolveRun=>{
      const child=spawn(process.execPath,[resolve(root,'server/node_modules/tsx/dist/cli.mjs'),'--conditions=development',resolve(root,'reports/reward-mastery-study-2026-09-25/run-bot-final.ts'),resolve(out,'jobs',job.id+'.json')],{cwd:root,windowsHide:true,env:{...process.env,DEBUG_REWARD_MULT:'1'}});
