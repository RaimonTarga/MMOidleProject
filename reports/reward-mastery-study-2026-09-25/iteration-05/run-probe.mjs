...
const out=resolve(here,process.argv[2]??'stall-probe');if(existsSync(out))throw Error('Fresh output required');mkdirSync(resolve(out,'jobs'),{recursive:true});
...
const harness=resolve(root,'reports/reward-mastery-study-2026-09-25/run-bot-stall-probe.ts');
...
const jobs=[];for(const intervene of [false,true]){const source=JSON.parse(readFileSync(resolve(report,'iteration-04/cross/jobs/candidate-closing-striker-balanced-c-V-s101033-at-node-t4-volcanic-01.json')));source.id='candidate-stall-'+(intervene?'avoidance-off':'trace');source.intervene=intervene;source.maxMs=30*60000;source.wallLimitMs=240000;source.output=resolve(out,source.id+'.json');jobs.push(source);}
...
writeFileSync(resolve(out,'manifest.json'),JSON.stringify({createdAt:new Date().toISOString(),root,workers:1,stopAtMastery:false,goal:'Fast end around 50 minutes; observe slower-build cost, not force all builds to 60 minutes.',jobs},null,2));
...
await Promise.all(Array.from({length:1},worker));const after=hashes();
