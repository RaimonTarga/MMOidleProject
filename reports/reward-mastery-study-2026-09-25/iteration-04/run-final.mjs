...
const out=resolve(here,process.argv[2]??'final-correction');if(existsSync(out))throw Error('Fresh output required');mkdirSync(resolve(out,'jobs'),{recursive:true});
...
const ids=['closing-striker-balanced-c-V-s101009','closing-conduit-balanced-a-T-s101009','closing-squire-heavy-b-D-s101009'];
const jobs=[];
for(const id of ids)for(const nodeId of ['node-t4-tundra-01','node-t4-desert-03'])for(const seed of [101009,101033]) {
 ...
 jobs.push({id:jobId,arm:'candidate',mode:'historical-loadout',nodeId,classRoot:cell.build.classRoot,cell,seed,gearIds:inventory.groups.find(x=>x.tier===4&&x.biome===group).reference.ids,maxMs:180*60000,stopAtMastery:true,wallLimitMs:240000,hitboxPath:resolve(main,'server/dist/hitbox/baked-hitboxes.json'),output:resolve(out,jobId+'.json')});
}
for(const original of [...jobs].filter(j=>j.classRoot!=='cadence-root')){const j=structuredClone(original);j.id+='-policy';j.cell.id+='-policy';j.output=resolve(out,j.id+'.json');if(j.classRoot==='cooldown-root')j.cell.stance='offensive-stance';else j.cell.runeRules=j.cell.runeRules.filter(r=>r.actionId!=='wait-for-regen');jobs.push(j);}
jobs.splice(0,jobs.length,...jobs.filter(j=>j.classRoot==='cadence-root'||(j.id.endsWith('-policy')&&((j.classRoot==='cooldown-root')||(j.classRoot==='summoner-root'&&j.nodeId.includes('desert'))))));
...
writeFileSync(resolve(out,'manifest.json'),JSON.stringify({createdAt:new Date().toISOString(),root,workers:4,stopAtMastery:false,goal:'Fast end around 50 minutes; observe slower-build cost, not force all builds to 60 minutes.',jobs},null,2));
...
async function worker(){while(next<jobs.length){const j=jobs[next++];let timedOut=false;const result=await new Promise(done=>{
 ...
 const timer=setTimeout(()=>{timedOut=true;if(process.platform==='win32')execFile('taskkill',['/PID',String(child.pid),'/T','/F'],{windowsHide:true},()=>{});else child.kill();},270000);
