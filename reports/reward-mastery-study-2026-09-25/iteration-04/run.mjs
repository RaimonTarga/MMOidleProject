...
const out=resolve(here,process.argv[2]??'cross');if(existsSync(out))throw Error('Fresh output required');mkdirSync(resolve(out,'jobs'),{recursive:true});
...
const jobs=[];
for(const id of ids)for(const nodeId of ['node-t4-volcanic-01','node-t4-tundra-01','node-t4-desert-03'])for(const seed of [101009,101033]) {
 ...
 jobs.push({id:jobId,arm:'candidate',mode:'historical-loadout',nodeId,classRoot:cell.build.classRoot,cell,seed,gearIds:inventory.groups.find(x=>x.tier===4&&x.biome===group).reference.ids,maxMs:30*60000,stopAtMastery:false,wallLimitMs:180000,hitboxPath:resolve(main,'server/dist/hitbox/baked-hitboxes.json'),output:resolve(out,jobId+'.json')});
}
