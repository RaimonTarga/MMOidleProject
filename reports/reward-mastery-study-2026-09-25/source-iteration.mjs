...
writeFileSync(resolve(here,'iteration-02/source-final.json'),JSON.stringify({recordedAt:new Date().toISOString(),sources,hitboxSha256:sha(resolve(root,'server/dist/hitbox/baked-hitboxes.json')),harnesses:Object.fromEntries(['run-bot.ts','run-bot-supported.ts','run-bot-final.ts','run-bot-survivors.ts'].map(p=>[p,sha(resolve(here,p))]))},null,2)+'\n');
