...
const out=resolve(here,'bot-run');mkdirSync(resolve(out,'jobs'),{recursive:true});
const inventory=JSON.parse(readFileSync(resolve(here,'economy-inventory.json'),'utf8'));
const cases=[[1,'plains'],[1,'cave'],[2,'forest'],[2,'desert'],[3,'swamp'],[3,'volcanic'],[4,'mountain'],[4,'trench']];
copyFileSync(resolve(here,'run-bot.ts'),resolve(candidate,'reports/reward-mastery-study-2026-09-25/run-bot.ts'));
const jobs=[];
for(const mode of ['fixed3','buy']) for(const [tier,biome] of cases) for(const classRoot of ['cadence-root','dot-root']) for(const arm of ['baseline','candidate']) {
  ...
  jobs.push({id,arm,mode,nodeId:`node-t${tier}-${biome}-01`,classRoot,seed:101009,gearIds:kit.ids,maxMs:[0,10,30,60,120][tier]*60000,
...
async function worker() {
  while(next<jobs.length) {
    ...
    const status=await new Promise(resolveRun=>{
      const child=spawn(process.execPath,[resolve(root,'server/node_modules/tsx/dist/cli.mjs'),'--conditions=development',resolve(root,'reports/reward-mastery-study-2026-09-25/run-bot.ts'),resolve(out,'jobs',job.id+'.json')],{cwd:root,windowsHide:true,env:{...process.env,DEBUG_REWARD_MULT:'1'}});
