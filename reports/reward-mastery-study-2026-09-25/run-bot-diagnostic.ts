...
try {
  ...
  while(elapsed<job.maxMs) {
    ...
    world.clearNodeEvents(target.nodeId);world.pendingDeaths=[]; world.worldLogJournal=[];world.worldLogByPlayer.clear(); if(elapsed%10000===0) console.log(JSON.stringify({simMs:elapsed,wallSeconds:(performance.now()-start)/1000}));
