...
| — | Conduit flavor pass | T1-T4 | 🔨 | in progress | `archive/summoner-flavor-pass-plan.md`, `conduit-current-state.md`, `conduit-player-specialization-sprites-handoff.md` | Summon-creature flavor work is complete: root retains its accepted baseline, and the frame/spec summon familiars are distinct, wired, packed, and covered. Still outstanding are nine bespoke tier-4 Conduit **player-character bodies**, one per specialization. The first Inquisitor/Marshal/Idolwright calibration was rejected as too similar to the accepted tier-2 bodies; do not rerun that recipe unchanged. Continue from the dedicated handoff, preferably in the rescued Claude session that retains the successful player-sprite reference context. Remaining balance gaps: Kilnmaster reads ~17px and `conduitDefenseShare` has no consumer. |
