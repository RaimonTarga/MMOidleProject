...
| — | Conduit flavor pass | T1-T4 | 🔨 | resolved | `summoner-flavor-pass-plan.md`, `conduit-current-state.md` | **NAMING + IDENTITY SHIPPED 2026-08-05.** Every branch renamed off placeholders: frames Splinter/Consort/Effigy, ranges Vigil/Procession/Harrier, nine tier-4 nodes renamed to professions (Inquisitor, Kilnmaster, Iconoclast, Marshal, Chorister, Ritualist, Covenanter, Champion, Idolwright). Display strings only — no id, tuning key, buff id or icon moved, so no migration. Summons stopped borrowing wildlife (hare/boar/frog/goat/T1 boss) for the Conduit's own bone skull; `MinionMonsterType` collapsed 8 ids → 1; base display size 48 → 28; 1px #14181a outline baked. Range is now tint + scale + attack FX (bolt/beam) and gates the lunge, never a body swap. Dead pre-overhaul tier-3 path system deleted (−1,675 lines). Masks lost a three-round PixelLab bake-off; skull geometry is the only legible form at 64px. REMAINING: 13 per-frame/per-spec summon bodies; Kilnmaster reads ~17px; `conduitDefenseShare` has no consumer. |
...
- **Living truth is `docs/cores-current-state.md`.** Plan + rationale:
  `docs/cores-rework-implementation-plan.md`. Design source: `design_docs/CORE_*.md`.
...
- Current state: see `docs/dungeon-current-state-and-gauntlet-plan.md`. Confirm what shipped vs. planned.
