...
| 12b | Monster combat rework (T1-T4) | all | 🔨 | resolved | `monster-combat-rework-current-state.md` + `design_docs/MONSTER_COMBAT_REWORK_HANDOFF_T1_T4_2026-08-22.md` | **IMPLEMENTED 2026-08-22** (structure + behavior; numbers are NOT). Follows Step 12: that pass gave every biome an ecology, this one gave individual monsters and LINEAGES a readable identity. A good implementation deleted more than it added — blanket evasion, roster-wide DoT, per-mob `rampOnCombat`, per-hit slow spam, universal anti-heal and the pack-alpha SCATTER all removed. Volcano Heat / Tundra Chill already existed as `ambientRamp` node features (Chill gained an attack-slow term); non-recursive corpses were already enforced. New primitives: `flies`, `staticSentry`, `idleAnchor`, `openingVolley`, `cadenceVolley`, `dotEffect.openerStacks`, `cadenceFinisher.rootMs`, `shellUp`, `empowersAllies`, `enemyShield.rechargeAfterCleanMs`, `enemyShield.shatter.vulnerability`, `scalesWithAmbientRamp.chargedOnly`, and four `chargedAttack` riders (`rootMs` / `appliesAntiheal` / `refreshesPlayerDots` / `requiresAmbientStacks`) so every periodic ability arrives with a cast bar instead of its own subsystem. Desert re-cast as EXACT 1:1 controller/dealer duos (dealers removed from the spawn pools so a lone kiter can never spawn). `charnel-brute` DEFERRED TO T5. Bosses untouched. Numbers = Step 15. **FOLLOW-UP PASS (uncommitted):** added `monsterAbilities` — an ordered list of independent cast-time abilities (hit / area-hit / attack-speed-buff / shield) so an elite gets a readable rotation without another subsystem; the Trench elites + T4 boss are its first users. It also absorbed two older, INVISIBLE primitives on Tundra/Volcano: `cadenceFinisher` (every-Nth-attack spike) became a named cast on its own cooldown, and the periodic `enemyShield` on the Volcano anchors / `elder-leviathan` became a casted `shield` action — the barrier now costs the monster a visible wind-up instead of reforming on a hidden timer. `openingVolley` is now supported-but-unauthored (Chameleon line moved to a recurring barrage). Also `concealedWhileIdle` and `castedAttackSpeedBuff.rallyNearby` (Jungle Ape Chestbeat — the one sanctioned exception to Jungle's no-pack rule). Invariant: ONE CAST PER MONSTER — the scheduler yields to a pending charged/buff/ward wind-up, because telegraphs are cleared by ownerId and the cast bar is keyed by monster id. **ECOLOGY POLISH 2026-09-11** (Volcano / Wasteland / Tundra, see `biome-ecology-current-state.md` §21): Volcano's locked "no packs, density is the swarm" call was OVERTURNED — it is now authored mixed packs (two anchor/catcher alphas per tier plus the T4 elite, leading the weak filler and the stationary gunner) via a new `pack.followerVariants` field that rolls one add-on group per spawn so a herd is not the same five monsters forever. The Gravewright became a pack alpha so its resurrection has ammunition from the first second of a pull. Tundra filled its two identity gaps (Frost Lurker gained RIME POUNCE, a committed `cast-charge-strike` lunge; the Mastodon's Frost-Tusk Impact became a planted `area-hit` circle with knockback at the SAME multiplier) and deepened the Yeti's Deep Freeze into a planted circle — deliberately making the T4 root MORE avoidable, since it is the biome's only root. Engine: `resolveChargedSlam` now applies charged-attack riders, closing a trap where `aoe` + `rootMs` silently rooted nobody. Densities and every stat UNCHANGED; the measured consequence is that packs replace loose spawns, so anchors/elites per node fall ~60% while fodder rises — one for the bot runs to judge. |
| 12c | Boss encounter rework (T1-T4) | all | 🔨 | resolved | `boss-encounter-rework-current-state.md` + `design_docs/BOSS_ENCOUNTER_REWORK_HANDOFF_T1_T4_2026-08-23.md` | **IMPLEMENTED 2026-08-23** (mechanics + structure; numbers are INHERITED, not re-pitched). 26 dungeon bosses across 11 lineages rebuilt so each is the apex expression of its BIOME's combat idea, and each tier DEEPENS that one idea rather than acquiring an unrelated keyword. Deleted the shared T2/T3/T4 template: `apply-soft-cap` + `shed-defense` (all four users), Tundra's 85%/70% ramp-cap lift, Volcanic's private `rampOnCombat`, Swamp T3's `attack x4`, Jungle/Trench `cadenceFinisher`, Plains' self-enrages, Mountain/Cave T1 timed shields, Desert T2's adds. **The anti-summon cleave is retired**: `targeting.prefersPlayers` drops minions from the aggro pool while a player is in reach (plus a re-acquire so sending summons in first does not defeat it), so `aoeAttack` survives on ONE boss (the Trench serpent, because it is the size of the arena). New generic seams: `empower-charged` (scale the signature telegraphed attack; multipliers COMPOSE across phases), `empower-shred` (Cave corrosion goes deeper, incl. patching the live stack), `raise-dead` (Wasteland burst resurrection on the existing corpse registry), `stoke-ramp` (Volcanic bends its node's Heat ramp: faster, floored, higher ceiling; cleared on boss death + node freeze), `spawn-pool`, `apply-shield.shatter`, `chargedAttack.healsSelfPct` (Trench Devour), `shellUp.repeatIntervalMs` (Volcanic T3 shell cycle). `CORPSE_TTL_MS` 15s -> 30s so Mass Resurrection has bodies. Identities: Plains swarm / Forest cadence (LOCKED) / Swamp rot / Mountain telegraphed impact / Cave erosion / Desert setup->punishment (mark at all 3 tiers) / Jungle Hunt->Frenzy / Tundra Chill+Ice Armor / Volcanic Heat race / Wasteland resurrection / Trench Devour duel. `void-overlord` untouched legacy. NEXT: re-run `bench/bossExam.ts` — the T1 band was measured with the old shields + cleave. **FOLLOW-UP PASS (uncommitted):** `spawn-adds` retired from Mountain T2, Cave T2, Cave T3, Jungle T2 and the Wasteland T4 opener, each replaced by a casted beat the boss performs itself (`cast.fx` gained `'shield'`); Volcanic T3/T4 `spawn-pool` wrapped in a cast. **Plains is now the only lineage using `spawn-adds` at all.** See §4a of the current-state doc |
...
| 15 | Balance + simulation tooling pass | all | 🔨 | resolved | `tier-balance-current-state.md` | **T1-T4 NUMERICAL BASELINE SET 2026-08-23** — the difficulty LADDER is now shaped and ordered at every tier (zero railroad overlaps, all four curves on target); bosses calibrated to one band per tier at a constant boss:trash ratio of x8.13 HP / x0.736 DPS. Locked: biome order per tier, x1.20/x1.20/x1.27/x1.32 danger steps, x1.26 eHP, a TWO-rung tier boundary (each tier drops two biomes off the bottom, so one rung made returning biomes *easier* each tier), and a cost/kill-only Trench. Tooling: `tier-table.ts` now models the charged-attack cycle, volleys, cadence finishers and `empowersAllies` — which exposed T1 Mountain sitting 42% over target and out-pressuring Caverns. **NOT done: absolute pitch.** Everything is a ratio; and `bench/bossExam.ts` cannot judge it because the reference player regressed to 0/30 wins on unchanged T1 bosses after the 2026-08-22 affinity/Barrier/Recovery/item/ability commits. Fixing that is the next task. Rewards + concurrency also unreconciled. |
...
- **FOLLOW-UP (uncommitted) — Charge became its own execution shape.** New `AbilityShape:
  "charge"`: a 400ms wind-up on the shared `beginAbilityCast` path (so cast speed applies and hard
  control breaks it), then `isChargingAbility` owns movement as a 4× rush until contact,
  interruption or `chargeMaxMs`. The speed is a temporary layer in `playerSpeedMults`, never a
  write to the player's position speed. `validateAbilities` now treats `cast` and `charge` as the
  two wind-up shapes and checks the charge fields — it previously rejected the whole roster
  because a non-`cast` shape authored `castMs`.
