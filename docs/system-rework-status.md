...
| 9 | Cores | T2+ | 🔨 | resolved | `cores-*.md` | IMPLEMENTED (machinery + 4 worked cores + rank chain). 5th `core` slot; `rangeTag` axis; directional cores OFF unless `selectedRange` matches (shared `coreIsActive`, gated in stats loop); cores OFF the +N track (getMaxUpgrade=0), rank up via Step 6 evolution (required-plus 0 for cores); T2 via biome-level placement; UI active/inactive indicator. Numbers + other biomes = your pass |
...
### 9. Cores  — 🔨 IMPLEMENTED (machinery + 4 worked cores + rank chain; paired docs `cores-*.md`)
