 ...
 *
 * AUTHORED BY THE DESIGNER, 2026-08-25. This is the reference "intended" walk;
 * the analytical tools are evidence for it, never authority over it. Do not
 * restructure it from run data without asking — a stall is a finding, not
 ...
 *   Mountain  armor + Brace + brace rune,  max out -> +3
 *   Swamp     charm + Cleanse + hazards,   max out -> +4
 ...
 * been learned — which is the whole point of the Mountain leg.
 */
 ...
 * purple). Total 6 RP against a starting budget of 8, so it always fits.
 */
...
export const STRIKER_T1: Route = {
  ...
  description:
    "Designer baseline for the Striker: gear through all five T1 biomes to Global Mastery 30, take every piece to +5, then clear all five T1 dungeon bosses with a per-boss loadout.",
  ...
  steps: [
    ...
    // Re-assert the loadout now that `fire-guard` finally has a Guard to fire.
    ...
    // No swamp armor or weapon; the charm is the only gear taken from here.
