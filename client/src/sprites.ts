...
export const BIOME_DECOR: Partial<Record<string, BiomeDecorArt[]>> = {
  ...
  forest: [
    {
      ...
      count: 18,
      variance: { min: 0.45, max: 1.5, group: 'undergrowth' },
      displayW: 54,
      displayH: 54,
      flipX: true,
      alpha: 0.95,
    ...
    },
    {
      ...
      count: 11,
      variance: { min: 0, max: 2.0, group: 'fungi' },
      displayW: 40,
      displayH: 40,
      flipX: true,
      alpha: 0.95,
    ...
    },
    {
      ...
      count: 20,
      variance: { min: 0.3, max: 1.7, group: 'litter' },
      displayW: 56,
      displayH: 56,
      flipX: true,
      alpha: 0.8,
  ...
  // open sand reads empty.
  //
  // EVERY spec sets `avoidsDirt`, which no other biome does. Hardpan here always means
  // ground the wind scoured bare — a rare pocket on most nodes, a caravan track on a few
  // (`shared/world/desertTracks.ts`) — and the whole point of scoured ground is that
  // nothing has settled on it. A pebble drift lying in the middle of a road is the tell
  // that the road was painted rather than travelled.
  //
  // Four variance groups, because the desolation lever in this biome is EMPTINESS rather
  // than a change of dressing: the backbone groups (stones, debris) swing wide but never
  // vanish, while the two character groups roll presence and do. A node can come out as
  // bare pan with nothing living and nothing stacked on it.
  desert: [
    {
      key: 'desert_stone_cluster',
      file: '/assets/environment/desert/stone-cluster.png',
      count: 11,
      variance: { min: 0.3, max: 1.7, group: 'stones' },
      displayW: 48,
      displayH: 48,
      flipX: true,
      alpha: 0.85,
    ...
    // The only living things in the kit, and the only group whose absence changes what
    // the node IS rather than how it is dressed: a node with no scrub is dead ground.
    {
      key: 'desert_dry_shrub',
      file: '/assets/environment/desert/dry-shrub.png',
      count: 9,
      variance: { min: 0.5, max: 2.0, presence: 0.6, group: 'scrub' },
      displayW: 50,
      displayH: 50,
      flipX: true,
      alpha: 0.9,
    ...
    },
    {
      ...
      count: 7,
      variance: { min: 0.25, max: 1.75, group: 'debris' },
      displayW: 52,
      displayH: 52,
      flipX: true,
      alpha: 0.85,
    ...
    // Stacked stone, the biggest props in the kit. Rare enough at count 2 that they read
    // as deliberate when present, so they get their own presence roll rather than riding
    // on the loose debris.
    {
      key: 'desert_stone_pile',
      file: '/assets/environment/desert/stone-pile.png',
      count: 2,
      variance: { min: 0.4, max: 2.2, presence: 0.55, group: 'cairns' },
      displayW: 74,
      displayH: 74,
      flipX: true,
      alpha: 0.95,
  ...
  //
  // Four variance groups. The two BACKBONE groups (undergrowth, vines) swing wide but
  // never vanish — a jungle floor without ferns is not a thinner jungle, it is a
  // different biome. The two CHARACTER groups roll presence and do vanish, so some
  // nodes carry no flowering accent and no mossy stone at all: the same node dressed
  // as deep growth rather than as a clearing edge.
  jungle: [
    {
      key: 'jungle_fern_clump',
      file: '/assets/environment/jungle/fern-clump.png',
      count: 25,
      variance: { min: 0.35, max: 1.65, group: 'undergrowth' },
      displayW: 54,
      displayH: 54,
      flipX: true,
      alpha: 0.95,
    ...
    },
    {
      ...
      count: 18,
      variance: { min: 0.2, max: 1.8, group: 'vines' },
      displayW: 56,
      displayH: 56,
      flipX: true,
      alpha: 0.85,
    ...
    },
    {
      ...
      count: 11,
      variance: { min: 0.5, max: 2.0, presence: 0.6, group: 'flowers' },
      displayW: 48,
      displayH: 48,
      flipX: true,
      alpha: 0.95,
    ...
    },
    {
      ...
      count: 2,
      variance: { min: 0.5, max: 2.2, presence: 0.5, group: 'stones' },
      displayW: 76,
      displayH: 76,
      flipX: true,
      alpha: 0.95,
  ...
  trench: [
    {
      ...
      count: 16,
      variance: { min: 0.3, max: 1.7, group: 'coral' },
      displayW: 48,
      displayH: 48,
      flipX: true,
      alpha: 0.95,
    ...
    },
    {
      ...
      count: 16,
      variance: { min: 0.35, max: 1.65, group: 'worms' },
      displayW: 44,
      displayH: 44,
      flipX: true,
      alpha: 0.95,
    ...
    },
    {
      ...
      count: 14,
      variance: { min: 0.3, max: 1.7, group: 'anemones' },
      displayW: 46,
      displayH: 46,
      flipX: true,
      alpha: 0.95,
    ...
    },
    {
      ...
      count: 24,
      variance: { min: 0.4, max: 1.6, group: 'silt' },
      displayW: 48,
      displayH: 48,
      flipX: true,
      alpha: 0.85,
    ...
    },
    {
      ...
      count: 3,
      variance: { min: 0.5, max: 2.5, presence: 0.5, group: 'whalebone' },
      displayW: 78,
      displayH: 78,
      flipX: true,
      alpha: 0.95,
  ...
  graveyard: [
    {
      ...
      count: 18,
      variance: { min: 0.35, max: 1.65, group: 'drifts' },
      displayW: 56,
      displayH: 56,
      flipX: true,
      alpha: 0.8,
    ...
    },
    {
      ...
      count: 11,
      variance: { min: 0.3, max: 1.7, group: 'thorns' },
      displayW: 52,
      displayH: 52,
      flipX: true,
      alpha: 0.95,
    ...
    },
    {
      ...
      count: 9,
      variance: { min: 0.4, max: 1.6, group: 'bones' },
      displayW: 46,
      displayH: 46,
      flipX: true,
      alpha: 0.9,
    ...
    },
    {
      ...
      count: 2,
      variance: { min: 0.5, max: 2.2, presence: 0.55, group: 'remains' },
      displayW: 74,
      displayH: 74,
      flipX: true,
      alpha: 0.9,
