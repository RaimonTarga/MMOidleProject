...
> Hotfix release. Content is identical to v0.3, with the fixes from this patch
> collected under **Hotfixes (v0.4 addendum)** at the bottom.
...
## Hotfixes (v0.4 addendum)

### Bug fixes
- **World travel between zones is reliable again.** Map navigation, auto-travel, party-following, and fleeing now cross node borders dependably instead of getting stuck on a border, bouncing back to the previous zone, or sliding along an edge into a corner. This covers diagonal routes and the northern border, which were the worst offenders.
- **Fixed the settings menu not scrolling.**
- **The debug panel is now hidden in production.**

### Technical notes
- Gate crossings now steer to the nearest border and drive through it, keep pathfinding waypoints out of the gate bands, pick the nearest walkable cell without directional bias, and force server-driven crossings near the target edge so corners can't deflect the trigger. This is an interim hotfix in `server/src/systems/world/transitions.ts` (and `nodePath.ts` / `navGrid.ts`) pending the in-progress netcode rework.
- Fixed the release script on Windows.
