...
function defensiveItemScore(item: ItemDefinition): number {
  return (item.statModifiers.maxHp ?? 0) + (item.statModifiers.plating ?? 0) * 5 + (item.statModifiers.damageReduction ?? 0) * 100 + (item.statModifiers.hpRegen ?? 0) * 4 + (item.mechanicEffects?.['defense.shield-pct'] ?? 0) * 100;
}
