...
// Two INDEPENDENT blocks: neither gates the other, so a local problem in one
// never consumes the other's allocation. Only a global identity fault stops the
// batch. The navigation watch runs on the Jungle block as existing protection,
// not as a new study; T1 Mountain nodes author no avoided feature.
const BLOCKS=[
 {name:'mountain-armor',cells:6,seeds:[75011,77003,79001],limitMs:35*60000,
  audits:['scripts/charged-cast-audit.mjs','scripts/guard-window-audit.mjs']},
...
for(const block of BLOCKS){
 ...
 const result=await run('server/scripts/ttkSurvey.ts',['--trial=durability36',`--block=${block.name}`,'--mode=run',`--out=${out}`,`--revision=${args.revision}`,`--hitboxes=${args.hitboxes}`],join(root,`${block.name}.log`),Math.min(remaining,block.limitMs));
 ...
 try {
  const check=verifySurvey(out,{trial:'durability36',revision:args.revision,mode:'run',definitionsHash:args.definitions,hitboxesSha256:args['hitbox-hash'],cells:block.cells,runs:block.cells*3,seeds:block.seeds,allowCensored:true});
