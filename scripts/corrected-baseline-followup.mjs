...
const {T4_CELLS:cells,T4_BLOCKS:blocks,T4_ID:experimentId}=require('../server/bench/balance/correctedBaselineSpec.ts');
const {T4_SNAPSHOTS:snapshots}=require('../server/bench/balance/correctedBaselineSpec.ts');
...
if(args.mode==='prepare'){
 assert(args.control && args.candidate && args.hitboxes&&!existsSync(packet));assert.equal(resolve(args.control),root);assert.equal(resolve(args.candidate),root);assert.equal(git('status','--porcelain','--untracked-files=no'),'');mkdirSync(packet,{recursive:true});
 ...
 write(join(packet,'manifest.json'),{experimentId,status:'prepared-unrun',planned:32,synthetic:true,economyEligible:false,seeds:[101009,101033],dtMs:100,farmingCapMs:600000,bossCapMs:300000,endpointsMs:[300000,600000,1200000],stopOnFirstDeath:true,watchdogs,snapshots,cases:cells,
...
let expected=[];
if(qualification){assert.equal(qualification.completed,32);assert.equal(qualification.combatObservations,0);assert.equal(qualification.manifestSha256,sha(readFileSync(join(packet,'manifest.json'))));assert.equal(qualification.receiptsSha256,sha(readFileSync(join(qualification.out,'resolved-builds.json'))));expected=json(join(qualification.out,'resolved-builds.json'));}
if(args.mode==='run'){const replay=json(join(packet,'receipt-checked.json'));assert.equal(replay.completed,32);assert.equal(replay.receiptsSha256,qualification.receiptsSha256);assert.equal(replay.manifestSha256,qualification.manifestSha256);assert.equal(sha(readFileSync(join(replay.out,'resolved-builds.json'))),replay.receiptsSha256);assert(existsSync(join(replay.out,'complete.json')));}
...
function publish(){const counts={planned:32,completed:rows.filter(r=>r.status==='complete').length,newCompleted:rows.filter(r=>r.status==='complete').length,dead:rows.filter(r=>r.status==='complete'&&['bot-died','player-died','simultaneous-terminal'].includes(r.outcome)).length,failed:rows.filter(r=>r.status==='failure').length,omitted:0,notRun:rows.filter(r=>r.status==='not-run').length,qualified:rows.filter(r=>r.status==='qualified').length};
...
const historical=json(join(root,'server/bench/balance/correctedBaselineReferences.json'));
function receipt(c,r){const h=historical.find(h=>h.observationId===c.id);assert(h);for(const key of ['declared','skillPath','equipment','mastery'])assert.deepEqual(r.packageReadback[key],h.receipt.ready.packageReadback[key],'Historical fixed input drift: '+key);
assert.equal(r.cell,c.id);assert.equal(r.seed,c.seed);assert.equal(r.view.hp,r.view.maxHp);assert.equal(r.view.barrier,r.view.barrierMax);if(c.progressionSnapshot){assert.deepEqual(r.packageReadback.progression.snapshot,c.progressionSnapshot);assert.equal(r.packageReadback.runicPoints.budget,c.progressionSnapshot.rp);}else {assert.equal(r.view.globalMastery,72);assert.equal(r.packageReadback.runicPoints.budget,30);}assert.equal(r.packageReadback.declared.stance,c.stance);assert.deepEqual(r.packageReadback.declared.abilities,c.abilities);assert.deepEqual(r.packageReadback.declared.runeRules,c.runeRules);
...
const counts=publish();const complete=args.mode!=='run'?counts.qualified===32:counts.completed===32;
write(join(out,complete?'complete.json':'partial.json'),{...counts,mode:args.mode,sourceCommit:frozen.sourceCommit,combatObservations:args.mode!=='run'?0:counts.completed,failure});
if(complete&&args.mode==='qualify')write(join(packet,'qualified.json'),{out,completed:32,combatObservations:0,manifestSha256:sha(readFileSync(join(packet,'manifest.json'))),receiptsSha256:sha(readFileSync(join(out,'resolved-builds.json')))});
if(complete&&args.mode==='receipt-check')write(join(packet,'receipt-checked.json'),{out,completed:32,combatObservations:0,manifestSha256:qualification.manifestSha256,receiptsSha256:sha(readFileSync(join(out,'resolved-builds.json')))});
