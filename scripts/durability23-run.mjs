...
const started=Date.now(),blocks=['volcanic','graveyard','jungle'];
...
for(const block of blocks){
 ...
 const result=await run('server/scripts/ttkSurvey.ts',['--trial=durability23',`--block=${block}`,'--mode=run',`--out=${out}`,`--revision=${args.revision}`,`--hitboxes=${args.hitboxes}`],join(root,`${block}.log`),Math.min(remaining,35*60000));
 ...
 const expectedCells={volcanic:24,graveyard:28,jungle:12}[block];
 const verified=verifySurvey(out,{trial:'durability23',revision:args.revision,mode:'run',definitionsHash:args.definitions,hitboxesSha256:args['hitbox-hash'],cells:expectedCells,runs:expectedCells*3,seeds:[44017,46021,48017],allowCensored:true});
