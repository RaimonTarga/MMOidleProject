...
const {PROGRESSION_CELLS:cells,PROGRESSION_BLOCKS:blocks,PROGRESSION_ID:experimentId}=require('../server/bench/balance/overnightProgressionSpec.ts');
...
if(args.mode==='prepare'){
 assert(args.hitboxes&&!existsSync(packet));assert.equal(git('status','--porcelain','--untracked-files=no'),'');mkdirSync(packet,{recursive:true});
 ...
 write(join(packet,'manifest.json'),{experimentId,status:'prepared-unrun',planned:720,synthetic:true,economyEligible:false,seeds:[101009,101033],dtMs:100,farmingCapMs:1200000,bossCapMs:300000,endpointsMs:[300000,600000,1200000],stopOnFirstDeath:true,watchdogs,snapshots,cases:cells,
 ...
 write(join(packet,'identity.json'),identity(args.hitboxes));write(join(packet,'seal.json'),Object.fromEntries(['manifest.json','identity.json'].map(p=>[p,sha(readFileSync(join(packet,p)))])));
 console.log('720 cases sealed; no combat.');process.exit(0);
}
const manifest=json(join(packet,'manifest.json')),frozen=json(join(packet,'identity.json'));
function verify(){for(const[p,h]of Object.entries(json(join(packet,'seal.json'))))assert.equal(sha(readFileSync(join(packet,p))),h);assert.deepEqual(identity(frozen.hitboxes),frozen,'Source/runtime drift');assert.deepEqual(manifest.cases,cells);assert.deepEqual(manifest.watchdogs,watchdogs);}
verify();if(args.mode==='verify'){console.log('720 cases, source, runtime and hitboxes verified.');process.exit(0);}
...
const rows=cells.map(c=>({observationId:c.id,block:c.block,identityId:c.identityId,snapshotId:c.snapshotId,fixture:c.nodeId,seed:c.seed,arm:c.arm,comparisonId:c.comparisonId,setting:c.role,sourceCommit:frozen.sourceCommit,status:'not-run',reason:null}));
const receipts=[],inventory=[];
function publish(){const counts={planned:720,completed:rows.filter(r=>r.status==='complete').length,newCompleted:rows.filter(r=>r.status==='complete').length,failed:rows.filter(r=>r.status==='failure').length,omitted:0,notRun:rows.filter(r=>r.status==='not-run').length,qualified:rows.filter(r=>r.status==='qualified').length};
 write(join(out,'results-summary.json'),{experimentId,mode:args.mode,sourceCommit:frozen.sourceCommit,counts,rows});write(join(out,'resolved-builds.json'),receipts);write(join(out,'raw-inventory.json'),inventory);
...
async function child(block,mode,dest){
 const b=blocks[block],boss=b.cells[0].role==='boss';mkdirSync(dest,{recursive:true});const childOut=join(dest,'artifacts');
 const argv=['--import',pathToFileURL(require.resolve('tsx')).href,'--conditions=development',`scripts/${boss?'bossScreen':'ttkSurvey'}.ts`,`--trial=${experimentId}`,`--block=${block}`,`--mode=${mode}`,`--revision=${frozen.sourceCommit}`,`--source-contract=${join(packet,'identity.json')}`,`--hitboxes=${frozen.hitboxes}`,`--out=${childOut}`];
 const fd=openSync(join(dest,'process.log'),'w');const cp=spawn(process.execPath,argv,{cwd:join(root,'server'),stdio:['ignore',fd,fd],windowsHide:true});let reason=null,lastProgress=Date.now(),previous=-1;
...
function receipt(c,r){assert.equal(r.cell,c.id);assert.equal(r.seed,c.seed);assert.equal(r.view.hp,r.view.maxHp);assert.equal(r.view.barrier,r.view.barrierMax);assert.deepEqual(r.packageReadback.progression.snapshot,c.progressionSnapshot);assert.equal(r.packageReadback.runicPoints.budget,c.progressionSnapshot.rp);assert.equal(r.packageReadback.declared.stance,c.stance);assert.deepEqual(r.packageReadback.declared.abilities,c.abilities);assert.deepEqual(r.packageReadback.declared.runeRules,c.runeRules);
 const rec={observationId:c.id,snapshotId:c.snapshotId,sourceCommit:frozen.sourceCommit,ready:r};
 const pair=receipts.find(x=>cells.find(y=>y.id===x.observationId).comparisonId===c.comparisonId);if(pair)assert.equal(pair.ready.initialRosterHash,r.initialRosterHash,'Paired ecology drift');
...
const counts=publish();const complete=args.mode==='qualify'?counts.qualified===720:counts.completed===720;
...
if(complete&&args.mode==='qualify')write(join(packet,'qualified.json'),{out,completed:720,combatObservations:0,manifestSha256:sha(readFileSync(join(packet,'manifest.json'))),receiptsSha256:sha(readFileSync(join(out,'resolved-builds.json')))});
