...
for(const block of ['trench','mountain','tundra','desert'])for(const mode of ['qualify','pilot']){
 const out=join(root,`${block}-${mode}`);
 const result=spawnSync(process.execPath,night5ChildArgs(source,'server/scripts/ttkSurvey.ts',['--trial=durability22',`--block=${block}`,`--mode=${mode}`,`--out=${out}`,`--hitboxes=${hitboxes}`]),{cwd:source,encoding:'utf8',timeout:180000,windowsHide:true});
 writeFileSync(join(root,`${block}-${mode}.log`),result.stdout+'\n'+result.stderr);
 assert.equal(result.status,0,result.stderr);
 const read=n=>JSON.parse(readFileSync(join(out,n),'utf8'));
 assert.equal(read('complete.json').runs,mode==='qualify'?24:12);
 const manifest=read('manifest.json');assert.equal(manifest.cells.length,24);assert(manifest.cells.every(c=>c.role===block));
 assert.deepEqual(manifest.seeds,[38011,40009,42013]);
 if(mode==='pilot'){
