...
const nodeOf = c => (/jungle-(\d\d)/.exec(c) ?? [, 'n/a'])[1];

/** species -> root -> seed -> clean ttk samples, plus engagement bookkeeping. */
const species = new Map();
const perObservation = [];

for (const row of index) {
  const root = rootOf(row.cell), node = nodeOf(row.cell);
  ...
  for (const t of targets) {
    const s = species.get(t.type) ?? {
      ...
      castsStarted: 0, castsFired: 0, byRoot: {},
    };
  ...
  perObservation.push({
    cell: row.cell, seed: row.seed, root, node, outcome: row.outcome,
    simMs: row.elapsedMs,
...
for (const s of species.values()) {
  // seed medians -> root medians -> equally weighted across roots.
  const rootMedians = {};
  for (const [root, seeds] of Object.entries(s.byRoot)) {
    const seedMedians = Object.values(seeds).map(v => median(v));
    rootMedians[root] = median(seedMedians);
  }
  const weighted = median(Object.values(rootMedians).filter(v => v !== null));
  table.push({
    type: s.type, name: s.name, maxHp: s.maxHp,
...
table.sort((a, b) => (b.maxHp ?? 0) - (a.maxHp ?? 0));
...
console.log(`${name}: ${perObservation.length} observations, ${table.length} species\n`);
console.log('species'.padEnd(22) + 'maxHp'.padEnd(8) + 'eng'.padEnd(6) + 'kill'.padEnd(6) + 'clean'.padEnd(7) + 'unres'.padEnd(7) + 'fired/eng'.padEnd(11) + 'cleanTTK s (equal-weight roots)  roots');
for (const s of table) {
  console.log(
    s.type.padEnd(22) + String(s.maxHp ?? '-').padEnd(8) + String(s.engaged).padEnd(6) +
    String(s.killed).padEnd(6) + String(s.cleanKills).padEnd(7) + String(s.unresolved).padEnd(7) +
    String(s.mechanicFiredPerEngagement ?? '-').padEnd(11) +
    String(s.cleanTtkMsEqualWeightRoots === null ? 'n/a' : (s.cleanTtkMsEqualWeightRoots / 1000).toFixed(2)).padEnd(33) +
