...
const started=Date.now(),blocks=['jungle']; // Reuse the unchanged Durability30 Jungle matrix.
writeFileSync(join(root,'batch-manifest.json'),JSON.stringify({source,...args,blocks,started:new Date().toISOString(),ceilingHours:3},null,2));
...
for(const block of blocks){
 const remaining=3*3600000-(Date.now()-started);if(remaining<=0)break;
 const out=join(root,block),start=new Date().toISOString();
 const result=await run('server/scripts/ttkSurvey.ts',['--trial=durability30',`--block=${block}`,'--mode=run','--navigation-diagnostics=true',`--out=${out}`,`--revision=${args.revision}`,`--hitboxes=${args.hitboxes}`],join(root,`${block}.log`),Math.min(remaining,35*60000));
 ...
 const expectedCells={trench:24,jungle:4}[block];
 const verified=verifySurvey(out,{trial:'durability30',revision:args.revision,mode:'run',definitionsHash:args.definitions,hitboxesSha256:args['hitbox-hash'],cells:expectedCells,runs:expectedCells*3,seeds:(block==='jungle'?[44017,46021,48017]:[86011,88001,90001]),allowCensored:true});
 for(const row of JSON.parse(readFileSync(join(out,'index.json'),'utf8'))) {
  const prefix=join(out,row.cell+'-s'+row.seed);
  const diagnostic=JSON.parse(readFileSync(prefix+'-navigation.json','utf8'));
  const profile=JSON.parse(readFileSync(prefix+'.cpuprofile','utf8'));
  assert.equal(diagnostic.cell,row.cell);assert.equal(diagnostic.seed,row.seed);
  assert(diagnostic.ticks.length>0&&profile.nodes.length>0&&profile.samples.length>0,'Missing diagnostic data');
 }
