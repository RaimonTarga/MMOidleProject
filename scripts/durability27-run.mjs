...
const started=Date.now(),blocks=['mountain','desert'];
...
for(const block of blocks){
 ...
 const result=await run('server/scripts/ttkSurvey.ts',['--trial=durability27',`--block=${block}`,'--mode=run',`--out=${out}`,`--revision=${args.revision}`,`--hitboxes=${args.hitboxes}`],join(root,`${block}.log`),Math.min(remaining,35*60000));
 ...
 const expectedCells={mountain:24,desert:24}[block];
 const verified=verifySurvey(out,{trial:'durability27',revision:args.revision,mode:'run',definitionsHash:args.definitions,hitboxesSha256:args['hitbox-hash'],cells:expectedCells,runs:expectedCells*3,seeds:[68023,70001,72019],allowCensored:true});
