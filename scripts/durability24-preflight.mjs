...
for(const block of ['graveyard'])for(const mode of ['qualify','pilot']){
 const count={graveyard:56}[block];
 const out=join(root,`${block}-${mode}`);
 const result=spawnSync(process.execPath,night5ChildArgs(source,'server/scripts/ttkSurvey.ts',['--trial=durability24',`--block=${block}`,`--mode=${mode}`,`--out=${out}`,`--hitboxes=${hitboxes}`]),{cwd:source,encoding:'utf8',timeout:180000,windowsHide:true});
 ...
 assert.deepEqual(manifest.seeds,[50021,52009,54001]);
 if(mode==='pilot'){
