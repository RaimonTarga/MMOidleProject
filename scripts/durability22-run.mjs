...
const started=Date.now(),blocks=['trench','mountain','tundra','desert'];
...
for(const block of blocks){
 ...
 const result=await run('server/scripts/ttkSurvey.ts',['--trial=durability22',`--block=${block}`,'--mode=run',`--out=${out}`,`--revision=${args.revision}`,`--hitboxes=${args.hitboxes}`],join(root,`${block}.log`),Math.min(remaining,35*60000));
 ...
 const verified=verifySurvey(out,{trial:'durability22',revision:args.revision,mode:'run',definitionsHash:args.definitions,hitboxesSha256:args['hitbox-hash'],cells:expectedCells,runs:expectedCells*3,seeds:[38011,40009,42013],allowCensored:true});
