...
const {T4_CELLS:cells,T4_BLOCKS:blocks,T4_ID:experimentId}=require('../server/bench/balance/itemAbilityOutlierSpec.ts');
const {T4_SNAPSHOTS:snapshots}=require('../server/bench/balance/itemAbilityOutlierSpec.ts');
...
const scheduling={softDeadlineMs:8*60*60*1000,finishActiveObservation:true,primaryCount:44,alternativeCount:0};
...
if(args.mode==='prepare'){
 ...
 write(join(packet,'manifest.json'),{experimentId,status:'prepared-unrun',planned:cells.length,synthetic:true,economyEligible:false,seeds:[101009,101033],dtMs:100,farmingCapMs:600000,bossCapMs:300000,endpointsMs:[300000,600000],stopOnFirstDeath:true,watchdogs,scheduling,snapshots,cases:cells,
 ...
 console.log('44 cases sealed; no combat.');process.exit(0);
}
...
verify();if(args.mode==='verify'){console.log('44 cases, source, runtime and hitboxes verified.');process.exit(0);}
...
function publish(){const counts={planned:cells.length,primary:44,alternative:0,completed:rows.filter(r=>r.status==='complete').length,newCompleted:rows.filter(r=>r.status==='complete').length,dead:rows.filter(r=>r.status==='complete'&&['bot-died','player-died','simultaneous-terminal'].includes(r.outcome)).length,failed:rows.filter(r=>r.status==='failure').length,omitted:0,notRun:rows.filter(r=>r.status==='not-run').length,qualified:rows.filter(r=>r.status==='qualified').length};
...
function receipt(c,r){assert.equal(r.cell,c.id);assert.equal(r.seed,c.seed);assert.equal(r.view.hp,r.view.maxHp);assert.equal(r.view.barrier,r.view.barrierMax);if(c.progressionSnapshot){assert.deepEqual(r.packageReadback.progression.snapshot,c.progressionSnapshot);assert.equal(r.packageReadback.runicPoints.budget,c.progressionSnapshot.rp);}else {assert.equal(r.view.globalMastery,72);assert.equal(r.packageReadback.runicPoints.budget,30);}assert.equal(r.packageReadback.declared.stance,c.stance);assert.deepEqual(r.packageReadback.declared.abilities,c.abilities);assert.deepEqual(r.packageReadback.declared.runeRules,c.runeRules);
 assert.deepEqual(r.packageReadback.rites,c.rites??[]);assert.equal(r.candidate,c.candidate??null);
...
try{if(args.mode!=='run'){
 ...
 for(const c of cells){if(Date.now()-launchAt>=scheduling.softDeadlineMs){stopReason='Soft scheduling deadline; active observation finished, remaining cases not run';break;}const row=rows.find(r=>r.observationId===c.id),family=c.role==='farm'?'farm':`boss-${c.boss}`;
  ...
  const dest=join(out,c.id);try{const childOut=await child(c.id,'run',dest),dir=join(childOut,`${c.id}-s${c.seed}`),ready=json(join(dir,'ready.json')),result=json(join(dir,'summary.json'));
    ...
    Object.assign(row,{status:'complete',outcome:result.outcome,elapsedMs:result.elapsedMs,completedKills:result.completedKills,firstKillMs:result.work?.firstKillMs??null,endpoints:c.role==='farm'?[300000,600000].map(atMs=>{const e=result.endpoints?.find(e=>e.atMs===atMs);return {atMs,work:e?.work??null,owner:e?.owner??null};}):null,intervals:result.intervals?.map(i=>({...i,killsPerMinute:i.observed&&i.kills!==null?i.kills/((i.endMs-i.startMs)/60000):null})),bossKilled:result.bossKilled,bossKillEvidence:result.bossKillEvidence,playerDeathEvidence:result.playerDeathEvidence,bossHpRemaining:result.bossHpRemaining,terminalOwner:result.terminalOwner,bossHpFractionRemoved:result.bossHpFractionRemoved,checkpoints:result.checkpoints,terminalCombat:result.terminalCombat,work:result.work,sustain:result.sustain,guards:result.guards,strategy:result.strategy,terminalTargets:result.terminalTargets,externalSummary:join(dir,'summary.json')});
