...
const BLOCKS=[
 {name:'jungle-durability',cells:24,seeds:[63011,65003,67001],limitMs:35*60000,watchNavigation:true},
 {name:'mountain-guard',cells:12,seeds:[69001,71003,73009],limitMs:35*60000},
];
...
for(const block of BLOCKS){
 ...
 const result=await run('server/scripts/ttkSurvey.ts',['--trial=durability34',`--block=${block.name}`,'--mode=run',`--out=${out}`,`--revision=${args.revision}`,`--hitboxes=${args.hitboxes}`],join(root,`${block.name}.log`),Math.min(remaining,block.limitMs));
 ...
 if(existsSync(join(out,'index.json'))){
  ...
  if(block.watchNavigation){
   ...
   if(gate.code===0&&existsSync(gatePath)){
    ...
    note({block:block.name,navigationWatch:parsed.navigationGate,scenarioExposure:parsed.scenarioExposure,balanceExposure:parsed.balanceExposure,
 ...
 try {
  const check=verifySurvey(out,{trial:'durability34',revision:args.revision,mode:'run',definitionsHash:args.definitions,hitboxesSha256:args['hitbox-hash'],cells:block.cells,runs:block.cells*3,seeds:block.seeds,allowCensored:true});
