...
const {T4_CELLS:cells,T4_BLOCKS:blocks,T4_ID:experimentId}=require('../server/bench/balance/t4ClosingSpec.ts');
const {T4_SNAPSHOTS:snapshots}=require('../server/bench/balance/t4ClosingSpec.ts');
...
const scheduling={softDeadlineMs:8*60*60*1000,finishActiveObservation:true,primaryCount:432,alternativeCount:28};
...
if(args.mode==='prepare'){
 ...
 write(join(packet,'manifest.json'),{experimentId,status:'prepared-unrun',planned:cells.length,synthetic:true,economyEligible:false,seeds:[101009,101033],dtMs:100,farmingCapMs:1200000,bossCapMs:300000,endpointsMs:[300000,600000,1200000],stopOnFirstDeath:true,watchdogs,scheduling,snapshots,cases:cells,
 ...
 console.log('460 cases sealed; no combat.');process.exit(0);
}
...
verify();if(args.mode==='verify'){console.log('460 cases, source, runtime and hitboxes verified.');process.exit(0);}
...
function publish(){const counts={planned:cells.length,primary:432,alternative:28,completed:rows.filter(r=>r.status==='complete').length,newCompleted:rows.filter(r=>r.status==='complete').length,dead:rows.filter(r=>r.status==='complete'&&['bot-died','player-died','simultaneous-terminal'].includes(r.outcome)).length,failed:rows.filter(r=>r.status==='failure').length,omitted:0,notRun:rows.filter(r=>r.status==='not-run').length,qualified:rows.filter(r=>r.status==='qualified').length};
