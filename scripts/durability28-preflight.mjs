...
for(const block of ['mountain2','desert2','mountain4'])for(const mode of ['qualify','pilot']){
 const count={mountain2:8,desert2:8,mountain4:8}[block];
 const out=join(root,`${block}-${mode}`);
 const result=spawnSync(process.execPath,night5ChildArgs(source,'server/scripts/ttkSurvey.ts',['--trial=durability28',`--block=${block}`,`--mode=${mode}`,`--out=${out}`,`--hitboxes=${hitboxes}`]),{cwd:source,encoding:'utf8',timeout:180000,windowsHide:true});
 ...
 const manifest=read('manifest.json');assert.equal(manifest.cells.length,count);assert(manifest.cells.every(c=>c.role===block.slice(0,-1)));
 assert.deepEqual(manifest.seeds,[74017,76001,78007]);
 if(mode==='pilot'){
