...
assert(dir,'Provide the completed Durability19 output directory');
...
const matchedSets=[...groups].map(([key,arms])=>{
  assert.equal(arms.length,2,'Incomplete matched treatment set');
  assert.equal(new Set(arms.map(r=>r.treatment)).size,2);
