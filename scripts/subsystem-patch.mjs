...
const {T4_CELLS:cells,T4_BLOCKS:blocks,T4_ID:experimentId}=require('../server/bench/balance/subsystemPatchSpec.ts');
const {T4_SNAPSHOTS:snapshots}=require('../server/bench/balance/subsystemPatchSpec.ts');
...
const scheduling={softDeadlineMs:8*60*60*1000,finishActiveObservation:true,primaryCount:24,alternativeCount:0};
...
if(args.mode==='prepare'){
 assert(args.control && args.candidate && args.hitboxes&&!existsSync(packet));assert.equal(resolve(args.control),root);assert.notEqual(resolve(args.candidate),root);assert.equal(git('status','--porcelain','--untracked-files=no'),'');mkdirSync(packet,{recursive:true});
...
function publish(){const counts={planned:cells.length,primary:24,alternative:0,completed:rows.filter(r=>r.status==='complete').length,newCompleted:rows.filter(r=>r.status==='complete').length,dead:rows.filter(r=>r.status==='complete'&&['bot-died','player-died','simultaneous-terminal'].includes(r.outcome)).length,failed:rows.filter(r=>r.status==='failure').length,omitted:0,notRun:rows.filter(r=>r.status==='not-run').length,qualified:rows.filter(r=>r.status==='qualified').length};
...
function receipt(c,r){assert.equal(r.cell,c.id);assert.equal(r.seed,c.seed);assert.equal(r.view.hp,r.view.maxHp);assert.equal(r.view.barrier,r.view.barrierMax);if(c.progressionSnapshot){assert.deepEqual(r.packageReadback.progression.snapshot,c.progressionSnapshot);assert.equal(r.packageReadback.runicPoints.budget,c.progressionSnapshot.rp);}else {assert.equal(r.view.globalMastery,72);assert.equal(r.packageReadback.runicPoints.budget,30);}assert.equal(r.packageReadback.declared.stance,c.stance);assert.deepEqual(r.packageReadback.declared.abilities,c.abilities);assert.deepEqual(r.packageReadback.declared.runeRules,c.runeRules);
 assert.deepEqual(r.packageReadback.rites,c.rites??[]);assert.equal(r.candidate??null,null);
