...
for(const block of ['mountain','desert'])for(const mode of ['qualify','pilot']){
 ...
 const result=spawnSync(process.execPath,night5ChildArgs(source,'server/scripts/ttkSurvey.ts',['--trial=durability27',`--block=${block}`,`--mode=${mode}`,`--out=${out}`,`--hitboxes=${hitboxes}`]),{cwd:source,encoding:'utf8',timeout:180000,windowsHide:true});
 ...
 assert.deepEqual(manifest.seeds,[68023,70001,72019]);
 if(mode==='pilot'){
