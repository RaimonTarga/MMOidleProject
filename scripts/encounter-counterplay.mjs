...
const {ENCOUNTER_CELLS:cells,ENCOUNTER_BLOCKS:blocks,ENCOUNTER_ID:experimentId}=require('../server/bench/balance/encounterCounterplaySpec.ts');
...
if(args.mode==='prepare'){
 ...
 write(join(packet,'manifest.json'),{experimentId,status:'prepared-unrun',planned:24,synthetic:true,economyEligible:false,seeds:[101009,101033],dtMs:100,farmingCapMs:600000,bossCapMs:300000,endpointsMs:[300000,600000],stopOnFirstDeath:true,watchdogs,snapshots,cases:cells,
...
function publish(){const counts={planned:24,completed:rows.filter(r=>r.status==='complete').length,newCompleted:rows.filter(r=>r.status==='complete').length,failed:rows.filter(r=>r.status==='failure').length,omitted:0,notRun:rows.filter(r=>r.status==='not-run').length,qualified:rows.filter(r=>r.status==='qualified').length};
...
const counts=publish();const complete=args.mode!=='run'?counts.qualified===24:counts.completed===24;
...
if(complete&&args.mode==='qualify')write(join(packet,'qualified.json'),{out,completed:24,combatObservations:0,manifestSha256:sha(readFileSync(join(packet,'manifest.json'))),receiptsSha256:sha(readFileSync(join(out,'resolved-builds.json')))});
