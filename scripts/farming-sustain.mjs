...
const { FARMING_SUSTAIN_CELLS: cells } = require('../server/bench/balance/farmingSustainSpec.ts');
...
const git = (...a) => execFileSync('git',a,{cwd:root,encoding:'utf8',maxBuffer:32*1020*1020}).trim();
...
if (args.mode === 'prepare') {
  ...
  write(join(packet,'manifest.json'),{experimentId:'farming-sustain-01',status:'prepared-unrun',seed:101003,dtMs:100,capMs:300000,
    planned:20,historicalReuse:[],synthetic:true,economyEligible:false,stopOnFirstDeath:true,cases:cells});
  ...
  console.log('Sealed 20 observations; no combat.'); process.exit(0);
}
...
function verify() {
  ...
  assert.deepEqual(manifest.cases,cells); assert.equal(cells.length,20);
}
verify();
...
if (qualification) {
  assert.equal(qualification.completed,20);
...
function row(c) {
  return {observationId:c.id,identityId:c.identityId,arm:c.arm,comparisonId:c.comparisonId,tier:c.tier,root:c.className,
    ...
    itemId:c.build.gearItemIds.recovery,defaultStanceId:c.stance,recoveryRule:c.policy==='recuperating'?c.runeRules.at(-1):null,displacedAbility:c.displacedAbility,
...
function publish() {
  ...
  const counts = {planned:20,completed:all.filter(r=>r.status==='complete').length,failed:all.filter(r=>r.status==='failure').length,
...
for (const c of cells) {
  ...
  const argv = [require.resolve('tsx/cli'),'--conditions=development','scripts/ttkSurvey.ts','--trial=farming-sustain-01',
  ...
  const child = spawnSync(process.execPath,argv,{cwd:join(root,'server'),encoding:'utf8',timeout:15*60*1000,maxBuffer:16*1020*1020});
  ...
  try {
    ...
    assert.equal(ready.view.activeStance,c.stance); assert.deepEqual(ready.view.attunedStances,[c.stance,...(c.additionalStances??[])]);
    ...
    const receipt = {observationId:c.id,packageReadback:ready.packageReadback,conduitProfile:ready.conduitProfile,
      ...
      initialView:ready.view,attunedStances:ready.view.attunedStances,sustainReadback:ready.sustainReadback};
    ...
    if (args.mode === 'qualify') r.status = 'qualified';
      ...
      r.firstKillDelayMs = s.targets.filter(t=>t.killedAtMs!==null).reduce((n,t)=>Math.min(n,t.killedAtMs),Infinity);
      if(!Number.isFinite(r.firstKillDelayMs)) r.firstKillDelayMs=null;
      r.units={time:'ms',health:'HP',hpFraction:'fraction'};
      r.recordedIncomingHpDamage=s.incomingDamage; r.recoveryEpisodes=s.recovery; r.deathEvidence=s.playerDeathEvidence;
      r.paths.sustainTransitions=`${detail}/sustain-transitions.json`;
      r.sustain = s.sustain ?? null; r.runicPoints = ready.packageReadback.runicPoints; r.terminalOwner = s.terminalOwner;
      ...
      for(const name of ['events.jsonl','samples.jsonl','sustain-transitions.json']) { const p = `${detail}/${name}`; const bytes = readFileSync(join(out,p)); rawInventory.push({observationId:c.id,path:p,bytes:bytes.length,sha256:sha(bytes)}); }
  ...
  rows.push(r);publish(); console.log(`${rows.length}/20 ${c.id}: ${r.status}`);
...
if (completed === 20) for (const c of cells) {
  const group = receipts.filter(r => cells.find(x => x.id === r.observationId).comparisonId === c.comparisonId);
  assert(group.every(r => r.initialRosterHash === group[0].initialRosterHash), 'Pair ecology drift');
}
write(join(out,'resolved-builds.json'),receipts);
write(join(out,completed===20?'complete.json':'partial.json'),{planned:20,completed,combatObservations:args.mode==='qualify'?0:completed});
if(args.mode==='qualify' && completed===20) write(join(packet,'qualified.json'),{out,completed,combatObservations:0,
...
if(completed!==20) process.exitCode=1;
