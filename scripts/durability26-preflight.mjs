...
for(const block of ['mountain','desert'])for(const mode of ['qualify','pilot']){
 const count={mountain:24,desert:24}[block];
 const out=join(root,`${block}-${mode}`);
 const result=spawnSync(process.execPath,night5ChildArgs(source,'server/scripts/ttkSurvey.ts',['--trial=durability26',`--block=${block}`,`--mode=${mode}`,`--out=${out}`,`--hitboxes=${hitboxes}`]),{cwd:source,encoding:'utf8',timeout:180000,windowsHide:true});
 ...
 assert.deepEqual(manifest.seeds,[62003,64007,66029]);
 if(mode==='pilot'){
