...
const started=Date.now(),blocks=['mountain2','desert2','mountain4'];
...
for(const block of blocks){
 ...
 const result=await run('server/scripts/ttkSurvey.ts',['--trial=durability28',`--block=${block}`,'--mode=run',`--out=${out}`,`--revision=${args.revision}`,`--hitboxes=${args.hitboxes}`],join(root,`${block}.log`),Math.min(remaining,35*60000));
 ...
 const expectedCells={mountain2:8,desert2:8,mountain4:8}[block];
 const verified=verifySurvey(out,{trial:'durability28',revision:args.revision,mode:'run',definitionsHash:args.definitions,hitboxesSha256:args['hitbox-hash'],cells:expectedCells,runs:expectedCells*3,seeds:[74017,76001,78007],allowCensored:true});
