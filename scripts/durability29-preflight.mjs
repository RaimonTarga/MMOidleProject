...
for(const block of ['mountain2','mountain4'])for(const mode of ['qualify','pilot']){
 const count={mountain2:24,mountain4:24}[block];
 const out=join(root,`${block}-${mode}`);
 const result=spawnSync(process.execPath,night5ChildArgs(source,'server/scripts/ttkSurvey.ts',['--trial=durability29',`--block=${block}`,`--mode=${mode}`,`--out=${out}`,`--hitboxes=${hitboxes}`]),{cwd:source,encoding:'utf8',timeout:180000,windowsHide:true});
 ...
 assert.deepEqual(manifest.seeds,[80021,82003,84011]);
 if(mode==='pilot'){
