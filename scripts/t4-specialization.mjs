...
const {T4_CELLS:cells,T4_BLOCKS:blocks,T4_ID:experimentId}=require('../server/bench/balance/t4SpecializationSpec.ts');
const {T4_SNAPSHOT:snapshot}=require('../server/bench/balance/t4SpecializationSpec.ts'); const snapshots={[snapshot.id]:snapshot};
...
if(args.mode==='prepare'){
 ...
 write(join(packet,'manifest.json'),{experimentId,status:'prepared-unrun',planned:432,synthetic:true,economyEligible:false,seeds:[101009,101033],dtMs:100,farmingCapMs:600000,bossCapMs:300000,endpointsMs:[300000,600000],stopOnFirstDeath:true,watchdogs,snapshots,cases:cells,
 ...
 console.log('432 cases sealed; no combat.');process.exit(0);
}
...
verify();if(args.mode==='verify'){console.log('432 cases, source, runtime and hitboxes verified.');process.exit(0);}
...
let expected=[];
if(qualification){assert.equal(qualification.completed,432);assert.equal(qualification.combatObservations,0);assert.equal(qualification.manifestSha256,sha(readFileSync(join(packet,'manifest.json'))));assert.equal(qualification.receiptsSha256,sha(readFileSync(join(qualification.out,'resolved-builds.json'))));expected=json(join(qualification.out,'resolved-builds.json'));}
if(args.mode==='run'){const replay=json(join(packet,'receipt-checked.json'));assert.equal(replay.completed,432);assert.equal(replay.receiptsSha256,qualification.receiptsSha256);assert.equal(replay.manifestSha256,qualification.manifestSha256);assert.equal(sha(readFileSync(join(replay.out,'resolved-builds.json'))),replay.receiptsSha256);assert(existsSync(join(replay.out,'complete.json')));}
...
function publish(){const counts={planned:432,completed:rows.filter(r=>r.status==='complete').length,newCompleted:rows.filter(r=>r.status==='complete').length,dead:rows.filter(r=>r.status==='complete'&&['bot-died','player-died','simultaneous-terminal'].includes(r.outcome)).length,failed:rows.filter(r=>r.status==='failure').length,omitted:0,notRun:rows.filter(r=>r.status==='not-run').length,qualified:rows.filter(r=>r.status==='qualified').length};
...
const counts=publish();const complete=args.mode!=='run'?counts.qualified===432:counts.completed===432;
...
if(complete&&args.mode==='qualify')write(join(packet,'qualified.json'),{out,completed:432,combatObservations:0,manifestSha256:sha(readFileSync(join(packet,'manifest.json'))),receiptsSha256:sha(readFileSync(join(out,'resolved-builds.json')))});
if(complete&&args.mode==='receipt-check')write(join(packet,'receipt-checked.json'),{out,completed:432,combatObservations:0,manifestSha256:qualification.manifestSha256,receiptsSha256:sha(readFileSync(join(out,'resolved-builds.json')))});
