...
for(const block of ['mountain','desert'])for(const mode of ['qualify','pilot']){
 const count={mountain:36,desert:36}[block];
 const out=join(root,`${block}-${mode}`);
 const result=spawnSync(process.execPath,night5ChildArgs(source,'server/scripts/ttkSurvey.ts',['--trial=durability25',`--block=${block}`,`--mode=${mode}`,`--out=${out}`,`--hitboxes=${hitboxes}`]),{cwd:source,encoding:'utf8',timeout:180000,windowsHide:true});
 ...
 assert.deepEqual(manifest.seeds,[56003,58013,60013]);
 if(mode==='pilot'){
