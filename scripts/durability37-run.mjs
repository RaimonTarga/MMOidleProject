...
// Durability37 is a REGRESSION of one integrated candidate, not a search. Neither
// block installs anything: both fight authored source, so there is no overlay to
// restore and no control arm. The two blocks are INDEPENDENT -- a local problem in
// one never consumes the other's allocation, and only a global identity fault
// stops the batch. The navigation watch runs on the Jungle ladder as the existing
// protection it has always been, not as a new study.
//
// Block I's seed count is 1, not 3: twenty cells, twenty observations. `runs` is
// therefore derived per block rather than assumed to be cells x 3.
const BLOCKS=[
 {name:'jungle-ladder',cells:18,seeds:[81013,83003,85009],limitMs:35*60000,
  watchNavigation:true,audits:['scripts/species-timing-audit.mjs','scripts/charged-cast-audit.mjs']},
 {name:'mob-integration',cells:20,seeds:[87011],limitMs:45*60000,
  audits:['scripts/species-timing-audit.mjs','scripts/charged-cast-audit.mjs']},
];
...
for(const block of BLOCKS){
 ...
 const result=await run('server/scripts/ttkSurvey.ts',['--trial=durability37',`--block=${block.name}`,'--mode=run',`--out=${out}`,`--revision=${args.revision}`,`--hitboxes=${args.hitboxes}`],join(root,`${block.name}.log`),Math.min(remaining,block.limitMs));
 ...
 if(existsSync(join(out,'index.json'))){
  ...
  for(const script of block.audits??[]){
  ...
  if(block.watchNavigation){
   ...
   if(gate.code===0&&existsSync(gatePath)){
    ...
    note({block:block.name,navigationWatch:parsed.navigationGate,scenarioExposure:parsed.scenarioExposure,
      note:parsed.navigationGate.status==='fail'
        ? 'NAVIGATION REGRESSION: this block\'s balance interpretation is invalid until explained'
        : 'navigation watch clean'});
   } else note({block:block.name,status:'navigation-watch-unavailable'});
  }
 }
 ...
 try {
  const check=verifySurvey(out,{trial:'durability37',revision:args.revision,mode:'run',definitionsHash:args.definitions,hitboxesSha256:args['hitbox-hash'],cells:block.cells,runs:block.cells*block.seeds.length,seeds:block.seeds,allowCensored:true});
  ...
  console.log(block.name,JSON.stringify({artifactVerified:true,navigationWatch:state[block.name].navigationGate?.status??'n/a'}));
 } catch(error) {
