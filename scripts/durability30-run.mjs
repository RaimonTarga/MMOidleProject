...
const started=Date.now(),blocks=['trench','jungle'];
...
const run=(script,argv,log,limitMs)=>new Promise((resolveRun,reject)=>{
 const stream=createWriteStream(log);const child=spawn(process.execPath,[...(script==='server/scripts/ttkSurvey.ts'&&argv.includes('--block=jungle')?['--cpu-prof','--cpu-prof-dir='+root,'--cpu-prof-name=jungle.cpuprofile']:[]),...night5ChildArgs(source,script,argv)],{cwd:source,stdio:['ignore','pipe','pipe'],windowsHide:true});
...
for(const block of blocks){
 ...
 const result=await run('server/scripts/ttkSurvey.ts',['--trial=durability30',`--block=${block}`,'--mode=run',`--out=${out}`,`--revision=${args.revision}`,`--hitboxes=${args.hitboxes}`],join(root,`${block}.log`),Math.min(remaining,35*60000));
