...
const started=Date.now(),blocks=['tundra','volcanic','trench'];
writeFileSync(join(root,'batch-manifest.json'),JSON.stringify({source,...args,blocks,started:new Date().toISOString(),ceilingHours:2},null,2));
...
for(const block of blocks){
 const remaining=2*3600000-(Date.now()-started);if(remaining<=0)break;
 const out=join(root,block),start=new Date().toISOString();
 const result=await run('server/scripts/ttkSurvey.ts',['--trial=durability21',`--block=${block}`,'--mode=run',`--out=${out}`,`--revision=${args.revision}`,`--hitboxes=${args.hitboxes}`],join(root,`${block}.log`),Math.min(remaining,35*60000));
 ...
 const expectedCells=12;
 const verified=verifySurvey(out,{trial:'durability21',revision:args.revision,mode:'run',definitionsHash:args.definitions,hitboxesSha256:args['hitbox-hash'],cells:expectedCells,runs:expectedCells*3,seeds:[32003,34019,36007],allowCensored:true});
