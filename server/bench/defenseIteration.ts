...
const capMs=180000;
const seeds=process.env.DEFENSE_FRESH_SEEDS==='1'?[101051,101063,101081,101093]:[101009,101033];
...
// Matched armor comparisons use fixed current cores and the imported Volcano tuning.
for(const cls of ['squire','striker','slinger','apprentice','spirit','conduit']) {
 for(const [tier,armor,home] of [[3,'desert-vest-t3','desert'],[3,'jungle-vest-t3','jungle'],[3,'cave-vest-t3','cave'],[4,'graveyard-vest-t4','graveyard'],[4,'trench-vest-t4','trench']] as const)
  for(const biome of [home,'volcanic'])add(tier,cls,armor,`node-t${tier}-${biome}-03`);
}
for(const cls of ['squire','slinger'])for(const biome of ['jungle','volcanic'])add(4,cls,'jungle-vest-t4',`node-t4-${biome}-03`);
...
for(const cell of selected)for(const seed of seeds){
 ...
 try{
  ...
  for(;now<capMs;){world.tick(100,now);now+=100;world.pendingDeaths=[];if(getResource(bot.tracksCombat,'engagementDrRemainingMs')>0)dawnActiveMs+=100;if(now%1000===0){samples.push({t:now,hp:bot.hasHealth.hp,debt:getResource(bot.tracksCombat,'damageDebtPool'),dawn:getResource(bot.tracksCombat,'engagementDrRemainingMs'),effects:structuredClone(bot.tracksCombat.statusEffects)});if(samples.length>30)samples.shift();}minHp=Math.min(minHp,bot.hasHealth.hp/bot.hasHealth.maxHp);if(bot.hasHealth.hp/bot.hasHealth.maxHp<.25)lowHpMs+=100;
  ...
  const row={id:cell.id,seed,outcome,elapsedMs:now,kills,bossSeen,bossHpFraction,minHp,lowHpMs,hpEnd:bot.hasHealth.hp,maxHit,fullHpLethalHits,damage,starting,lastHits:hits,deathCause,dawnActiveMs,samples};
