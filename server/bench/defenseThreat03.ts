...
writeFileSync(resolve(out,'manifest.json'),JSON.stringify({source:execFileSync('git',['rev-parse','HEAD'],{encoding:'utf8'}).trim(),diff:execFileSync('git',['diff'],{encoding:'utf8',maxBuffer:20e6}),runnerSource:readFileSync(__filename,'utf8'),scope:'Synthetic fixed direct-hit stream, no AI, movement, offensive kills, automatic abilities or environmental hazards. 60-second cap, passive recovery/debt/shields and native defensive pipeline. Equal gross DPS between profiles. Not farming/boss evidence.'},null,2));
let completed=0;
const nowOriginal=Date.now;let now=0;Date.now=()=>now;
try{
for(const cls of ['squire','striker','slinger','apprentice','spirit','conduit'])for(const tier of [3,4])for(const family of ['desert','jungle'])for(const plus of [0,3,5])for(const profile of ['small','large'])for(const variant of (family==='desert'?['baseline','desert-dr']:['baseline','jungle-frequency','jungle-strength'])){
 ...
 try{
  const c=structuredClone(BREADTH_CELLS.find(c=>c.tier===tier&&c.className===cls&&c.frame==='balanced'&&c.role==='farm'&&!c.controlCaseId&&!c.id.includes('-far-'))!);assert(c);
  c.build.gearItemIds.armor=id;c.upgradeLevel=plus;
  ...
  for(;now<60000;now+=100){
   updateCombatState(world,100);updateDefensiveSystems(world,100,now);
  ...
  appendFileSync(resolve(out,'rows.jsonl'),JSON.stringify({cls,tier,family,plus,profile,variant,elapsedMs:now,hp:bot.hasHealth.hp,maxHp:bot.hasHealth.maxHp,dead:bot.hasHealth.hp<=0||!!bot.isDead,hits,evades,damage,maxHit,minHp})+'\n');completed++;
 }finally{ITEM_DATABASE.set(id,original);teardownArena(world);}
}
writeFileSync(resolve(out,'complete.json'),JSON.stringify({completed,expected:360}));assert.equal(completed,360);
