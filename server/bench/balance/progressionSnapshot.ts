...
import { ABILITY_RECIPE_DATABASE, STANCE_RECIPE_DATABASE, RUNE_RECIPE_DATABASE, STARTER_RUNE_IDS,
  RITE_RECIPE_DATABASE, isRiteRecipeUnlocked, RECIPE_DATABASE, ITEM_DATABASE, globalMastery, runeBudgetForGlobalMastery, biomeLevelCap,
...
export function applyProgressionSnapshot(bot:PlayerEntity,cell:SurveyCell) {
  ...
  for(const id of cell.rites ?? []) {
    assert(id !== 'blood-offering');
    const r=[...RITE_RECIPE_DATABASE.values()].find(r=>r.riteId===id)!;
    assert(r && s.tier>=r.tier && isRiteRecipeUnlocked(r,p), `Unearned Rite ${id}`);
    pay(r.id,'learn-rite',r.cost,r.catalystCost);p.knownRites.push(id);
  }
