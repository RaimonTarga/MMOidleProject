...
writeFileSync(resolve(out,'manifest.json'),JSON.stringify({source:execFileSync('git',['rev-parse','HEAD'],{encoding:'utf8'}).trim(),diff:execFileSync('git',['diff'],{encoding:'utf8',maxBuffer:20e6}),runnerSource:readFileSync(__filename,'utf8'),scope:'Synthetic fixed direct-hit stream, no AI, movement, offensive kills or environmental hazards. Heavy frame and Tanking Stance. Explicit Guard on/off: 45% base strength amplified by authored Guard potency, 3.5 seconds every 6 seconds; synthetic schedule, not Rune AI or cooldown evidence. 60-second cap, passive recovery/debt/shields and native defensive pipeline. Burst stream: 1920 gross every 20 seconds at T4, same 96 gross DPS and 5760 total over 60 seconds as the previous small/large streams. Not farming/boss evidence.'},null,2));
let completed=0;
const nowOriginal=Date.now;let now=0;Date.now=()=>now;
try{
for(const cls of ['squire','striker'])for(const tier of [4])for(const family of ['mountain','tundra','graveyard'])for(const plus of [3,5])for(const profile of ['burst'])for(const core of ['core-arcanist','core-juggernaut'])for(const guard of [false,true])for(const variant of ['baseline']){
 ...
 try{
  ...
  const interval=20000,gross=1200*(tier===4?1.6:1);
...
writeFileSync(resolve(out,'complete.json'),JSON.stringify({completed,expected:48}));assert.equal(completed,48);
