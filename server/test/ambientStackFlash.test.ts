...
for (const effectId of ['volcanic-heat', 'tundra-chill'] as const) {
  ...
  assert.equal(ambientStackFlashTint(flash, 150, 0xffffff), peak, 'peak holds briefly so the pulse is readable');
  ...
  assert.equal(ambientStackFlashTint(repeat, 1100 + AMBIENT_STACK_FLASH_MS, 0xffffff), null, 'repeated pulses do not lengthen');
}
