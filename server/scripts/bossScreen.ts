import { T4_BLOCKS, T4_ID, assertT4Definitions } from '../bench/balance/t4SpecializationSpec';
...
const TRIALS: Record<string, {
  ...
  [T4_ID]: { defaultBlock:'none', blocks:T4_BLOCKS, assertDefinitions:assertT4Definitions,
    perBlock:Object.fromEntries(Object.entries(T4_BLOCKS).filter(([,b])=>b.cells[0].role==='boss').map(([id,b])=>[id,{bossId:b.cells[0].boss!,capMs:300000,seeds:[b.cells[0].seed],escorts:{}}])),
  },
  ...
  'player-breadth': {
    ...
    perBlock: Object.fromEntries(Object.entries(BREADTH_BLOCKS).filter(([,b]) => b.cells[0].role === 'boss').map(([id,b]) => [id, {bossId:b.cells[0].targetTypes[0],capMs:FAST_PASS_CAP_MS,seeds:[FAST_PASS_SEED],escorts:{}}])),
    assertDefinitions: assertBreadthDefinitions,
  },
  'player-package-fit': {
...
function run(cell: Night5Cell, seed: number) {
  ...
  try {
    ...
    if(['player-fast-pass', 'player-package-fit', 'player-breadth', PROGRESSION_ID, ENCOUNTER_ID, T4_ID].includes(trial)) assertFastPassHitboxes([bot, ...world.monsterEntitiesInNode(cell.nodeId)]);
