...
function run(cell:SurveyCell,seed:number) {
  ...
  try {
    ...
    if(args.trial==='durability33'||args.trial==='durability32') for(const type of (cell as Night5Cell).targetTypes) assert(initial.some(m=>m.type===type),'Missing target '+type);
...
try {
  const pilotIds:Record<string,string[]>={
    ...
    durability33:night5?.pilotIds??[], durability32:night5?.pilotIds??[], durability30:night5?.pilotIds??[], durability29:night5?.pilotIds??[], durability28:night5?.pilotIds??[], durability27:night5?.pilotIds??[], durability26:night5?.pilotIds??[], durability25:night5?.pilotIds??[], durability24:night5?.pilotIds??[], durability23:night5?.pilotIds??[],
  ...
  observations: for(const cell of cells) for(const seed of mode==='qualify'||mode==='pilot'?[trialSeeds[0]]:trialSeeds) {
    if(night5&&mode==='run'&&realNow()-batchWallStart>=(['durability21','durability22','durability23','durability24','durability25','durability26','durability27','durability28','durability29','durability30','durability32','durability33'].includes(args.trial)?30:75)*60*1000){budgetStopped=true;break observations;}
