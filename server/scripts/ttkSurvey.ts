...
function run(cell:SurveyCell,seed:number) {
  ...
  try {
    ...
    if(args.trial==='durability34'||args.trial==='durability33'||args.trial==='durability32') for(const type of (cell as Night5Cell).targetTypes) assert(initial.some(m=>m.type===type),'Missing target '+type);
...
try {
  ...
  observations: for(const cell of cells) for(const seed of mode==='qualify'||mode==='pilot'?[trialSeeds[0]]:trialSeeds) {
    if(night5&&mode==='run'&&realNow()-batchWallStart>=(['durability21','durability22','durability23','durability24','durability25','durability26','durability27','durability28','durability29','durability30','durability32','durability33','durability34'].includes(args.trial)?30:75)*60*1000){budgetStopped=true;break observations;}
