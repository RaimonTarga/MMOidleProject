import { installOutlierCandidate, type OutlierCell } from '../bench/balance/itemAbilityOutlierSpec';
import { T4_BLOCKS, T4_ID, assertT4Definitions } from '../bench/balance/itemAbilityOutlierSpec';
...
function run(cell:SurveyCell,seed:number) {
  ...
  const candidateOverlay=t4?installOutlierCandidate(cell as OutlierCell):null;
  ...
  try {
    ...
    const ready={...(t4?{candidate:(cell as OutlierCell).candidate??null,resolvedPassives:{...bot.usesSkills.passives}}:{}),...((guardCoverage||desert||progression)?{guardReadback:guardCoverageReadback(bot)}:{}),...(desert?{targetingReadback:desertTargetingReadback(bot)}:{}),cell:cell.id,seed,synthetic:true,view,
      ...
      ...(fastPass ? {packageReadback:fastPassReadback(cell,bot,view.globalMastery), definitionsIdentity:{base:manifest.definitionsHash,live:checkpointDefinitionsHash(),treated:!!(t4 && (cell as OutlierCell).candidate)}} : {}),initialRoster:initial,initialRosterHash:sha(JSON.stringify(initial)),
    ...
    const strategy = (desert || (t4 && cell.nodeId==='node-t4-desert-03')) ? new DesertStrategyRecorder(world,bot) : null;
    const guards = (guardCoverage||desert||progression||t4) ? new GuardCoverageRecorder(world,bot) : null;
    ...
    for(;elapsed<windowMs;elapsed+=100) {
      ...
      if(endurance) {
        ...
        if((t4?[300000,600000,1200000]:encounter?[300000,600000]:progression?PROGRESSION_ENDPOINTS:tundraClassFrame?TUNDRA_CLASS_FRAME_ENDPOINTS:desert?DESERT_ENDPOINTS:ENDURANCE_ENDPOINTS).includes(measuredMs) && !bot.isDead && bot.hasHealth.hp>0) {
        ...
        if(realNow()-heartbeatAt>=5000 || (t4?[300000,600000,1200000]:encounter?[300000,600000]:progression?PROGRESSION_ENDPOINTS:tundraClassFrame?TUNDRA_CLASS_FRAME_ENDPOINTS:desert?DESERT_ENDPOINTS:ENDURANCE_ENDPOINTS).includes(measuredMs)) {
  ...
  } finally {try {teardownArena(world);} finally {candidateOverlay?.restore();overlay?.restore();Date.now=realNow;Math.random=realRandom;}}
}
async function main() {
