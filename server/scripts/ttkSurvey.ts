...
try {
  const pilotIds:Record<string,string[]>={
    ...
    durability27:night5?.pilotIds??[], durability26:night5?.pilotIds??[], durability25:night5?.pilotIds??[], durability24:night5?.pilotIds??[], durability23:night5?.pilotIds??[],
  ...
  observations: for(const cell of cells) for(const seed of mode==='qualify'||mode==='pilot'?[trialSeeds[0]]:trialSeeds) {
    if(night5&&mode==='run'&&realNow()-batchWallStart>=(['durability21','durability22','durability23','durability24','durability25','durability26','durability27'].includes(args.trial)?30:75)*60*1000){budgetStopped=true;break observations;}
