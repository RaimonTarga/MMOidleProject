...
function run(cell:SurveyCell,seed:number) {
  ...
  try {
    ...
    if(args.trial==='durability15') for(const type of (cell as Durability15Cell).targetTypes) assert(initial.some(m=>m.type===type),'Missing target '+type);
