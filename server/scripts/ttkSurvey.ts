  ...
  try {
    ...
    for(;elapsed<windowMs;elapsed+=100) {
      ...
      if(endurance) {
        ...
        if((encounter?[300000,600000]:progression?PROGRESSION_ENDPOINTS:tundraClassFrame?TUNDRA_CLASS_FRAME_ENDPOINTS:desert?DESERT_ENDPOINTS:ENDURANCE_ENDPOINTS).includes(measuredMs) && !bot.isDead && bot.hasHealth.hp>0) {
        ...
        if(realNow()-heartbeatAt>=5000 || (encounter?[300000,600000]:progression?PROGRESSION_ENDPOINTS:tundraClassFrame?TUNDRA_CLASS_FRAME_ENDPOINTS:desert?DESERT_ENDPOINTS:ENDURANCE_ENDPOINTS).includes(measuredMs)) {
