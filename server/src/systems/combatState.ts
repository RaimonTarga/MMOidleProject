...
// StatusEffect type now lives in @mmo-idle/shared/components/effects so pure DoT
// damage formulas can take it as input. Server keeps a re-export so existing
// importers don't change paths.
...
export function updateCombatState(world: World, dt: number): void {
  for (const e of world.playerEntities) {
    tickCooldowns(e.combatState, dt);
    tickStatusEffectDurations(e.combatState, dt);
  }
