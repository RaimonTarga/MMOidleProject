import { getResource, setResource, type TracksCombat } from '@mmo-idle/shared';

// ── Max capacity ──────────────────────────────────────────────────────────────

/** Returns the declared maximum for a resource, or Infinity if none is set. */
export function getMaxResource(state: TracksCombat, key: string): number {
...
export function setMaxResource(state: TracksCombat, key: string, value: number): void {
 ...
 */
export function gainResource(state: TracksCombat, key: string, amount: number): number {
 ...
 */
export function spendResource(state: TracksCombat, key: string, amount: number): boolean {
  const current = getResource(state, key);
...
export function isResourceFull(state: TracksCombat, key: string): boolean {
...
export function isResourceEmpty(state: TracksCombat, key: string): boolean {
 ...
 */
export function getResourcePercent(state: TracksCombat, key: string): number {
  const max = state.resourceMaxes[key];
  if (!max) return 0;
...
export function resetResource(state: TracksCombat, key: string): void {
