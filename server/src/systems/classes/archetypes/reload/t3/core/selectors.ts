...
export function getSnipeReady(entity: PlayerEntity, world: World): boolean {
  ...
  const fullHpThreshold = entity.usesSkills.passives['reload.snipe-fullhp-threshold'] ?? FULL_HP_THRESHOLD;
  return !!target && target.hasHealth.hp >= target.hasHealth.maxHp * fullHpThreshold;
}
