 ...
 */
function findAggro(monster: MonsterEntity, world: World): AggroCandidate | null {
...
function resolveAggroTarget(world: World, monster: MonsterEntity): ResolvedAggroTarget | null {
  ...
  if (aggro.targetKind === 'player') {
    ...
    if (!p || p.hasPosition.nodeId !== monster.hasPosition.nodeId) return null;
...
function aggroAttackTargetId(target: ResolvedAggroTarget): string {
  return target.kind === 'player' ? target.entity.isPlayer.id : target.entity.isMinion.id;
}
...
function aggroSourceFromCandidate(c: AggroCandidate): { id: string; kind: AggroTargetKind } {
